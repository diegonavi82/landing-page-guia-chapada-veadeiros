...
async function main() {
  ...
  try {
    console.log('[admin-dash] web root:', await cdWebRoot(client));
    for (const rel of FILES) {
      ...
      console.log('[admin-dash] ↑', rel);
    }
  } finally {
    client.close();
  }
  const url = 'https://www.guiachapadaveadeiros.com/assets/js/gcv-dashboard.js?v=1.0.74';
  const res = await fetch(url);
  const text = await res.text();
  console.log('[admin-dash] HTTP', res.status, 'admin-first', text.includes("indexOf('admin')"));
}

main().catch((e) => {
  console.error('[admin-dash] ERRO:', e.message || e);
  process.exit(1);
});
