...
function gcv_cms_guide_fetch(int $userId): ?array
{
    $stmt = db()->prepare(
        'SELECT u.id AS user_id, u.name, u.email, u.role, u.status, u.avatar_url,
                ...
                g.bio_pt, g.bio_en, g.bio_es, c.name AS base_city_name
         FROM gcv_users u
...
if ($method === 'GET') {
    ...
    $rows = db()->query(
        'SELECT u.id AS user_id, u.name, u.email, u.status, g.nickname, g.full_name, g.phone, g.phone_ddi,
                g.cadastur, g.pix_key_type, g.photo_3x4_url, g.photo_url, c.name AS base_city_name
         FROM gcv_users u
         LEFT JOIN gcv_guides g ON g.user_id = u.id
         LEFT JOIN gcv_cities c ON c.id = g.base_city_id
         WHERE u.role = "guide"
...
if ($method === 'POST') {
    ...
    if ($diploma === '') $missing[] = 'Diploma do guia';
    if ($photo34 === '') $missing[] = 'Foto 3x4';
    ...
    try {
        ...
        $stmt->execute([$nickname ?: $fullName, $email, $hash, 'guide', $status]);
        ...
        $stmt = db()->prepare(
            'INSERT INTO gcv_guides (
              ...
              association_doc_url, photo_3x4_url, bio_pt, approved_at, approved_by
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
        );
        $stmt->execute([
            $userId,
            ...
            trim((string)($body['photo_url'] ?? $photo34)) ?: $photo34,
            $diploma,
...
if ($method === 'PUT') {
    ...
    $diploma = trim((string)($body['diploma_url'] ?? $current['diploma_url'] ?? ''));
    ...
    db()->beginTransaction();
    try {
        db()->prepare('UPDATE gcv_users SET name=?, email=?, status=? WHERE id=?')
            ->execute([$nickname ?: $fullName, $email, $status, $userId]);
        ...
        if (!empty($current['guide_id'])) {
            db()->prepare(
                'UPDATE gcv_guides SET nickname=?, full_name=?, phone=?, phone_ddi=?, phone_iso=?, birth_date=?,
                 ...
                 diploma_url=?, association_doc_url=?, photo_3x4_url=?, bio_pt=? WHERE user_id=?'
            )->execute([
                $nickname,
                $fullName,
                $phone,
                ...
                $birth,
                $cityId,
                trim((string)($body['cadastur'] ?? $current['cadastur'] ?? '')) ?: null,
                $pixKey,
                $pixType,
                trim((string)($body['pix_holder_name'] ?? $fullName)) ?: $fullName,
                trim((string)($body['photo_url'] ?? $current['photo_url'] ?? $photo34)) ?: $photo34,
                $diploma,
                trim((string)($body['association_doc_url'] ?? $current['association_doc_url'] ?? '')) ?: null,
                $photo34,
            ...
            db()->prepare(
                'INSERT INTO gcv_guides (
                  user_id, nickname, full_name, phone, phone_ddi, phone_iso, birth_date, base_city_id,
                  cadastur, pix_key, pix_key_type, pix_holder_name, photo_url, diploma_url,
                  association_doc_url, photo_3x4_url, bio_pt
                ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
            )->execute([
                ...
                'br', $birth, $cityId,
                trim((string)($body['cadastur'] ?? '')) ?: null,
                $pixKey, $pixType, $fullName, $photo34, $diploma,
                trim((string)($body['association_doc_url'] ?? '')) ?: null,
                $photo34, ($body['bio_pt'] ?? null),
            ]);
        }
        db()->commit();
    } catch (Throwable $e) {
