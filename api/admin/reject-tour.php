...
$stmt = db()->prepare(
    'SELECT t.id, t.title_pt, u.name AS guide_name, u.email AS guide_email
...
db()->prepare('UPDATE gcv_tours SET status = \'rejected\' WHERE id = ?')->execute([$tourId]);
mail_tour_rejected($tour['guide_email'], $tour['guide_name'], $tour['title_pt'], $reason);

json_response(true, ['message' => 'Passeio rejeitado']);
