...
function gcv_receipt_email_links_footer(string $code, string $locale): string
{
    ...
    return '<div style="max-width:920px;margin:24px auto 32px;padding:20px 24px;background:#ecfdf5;border:1px solid #a7f3d0;border-radius:12px;font-family:Inter,Arial,sans-serif;">'
...
$html = gcv_clean_string($data['html'] ?? '', 120000);
...
$subjects = [
    ...
    'pt' => 'Recibo Pix — Reserva ' . $code,

    'en' => 'Pix receipt — Reservation ' . $code,

    'es' => 'Recibo Pix — Reserva ' . $code,
...
$wrapped = '<!DOCTYPE html><html lang="' . htmlspecialchars($locale, ENT_QUOTES, 'UTF-8') . '"><head><meta charset="utf-8">'

    . '<title>' . htmlspecialchars($subject, ENT_QUOTES, 'UTF-8') . '</title></head><body style="margin:0;padding:0;background:#f5f5f5;">'

    . '<div style="display:none;max-height:0;overflow:hidden;">' . htmlspecialchars($preheader, ENT_QUOTES, 'UTF-8') . '</div>'

    . $html

    . gcv_receipt_email_links_footer($code, $locale)
...
$sent = send_mail($email, $subject, $wrapped, '', false);
